...
public class GameController : MonoBehaviour {
    ...
    void Update() {
        ...
        if (!gameOver && !gameClear && isPaused && Input.GetKeyDown(KeyCode.P)) Resume();
            ...
            timeValue.text = String.Format("{0}:{1}:{2}", Mathf.RoundToInt((time % (60 * 60 * 60)) / (60 * 60)).ToString(), Mathf.RoundToInt((time % (60 * 60)) / 60).ToString(), Mathf.RoundToInt(time % 60).ToString());
            levelValue.text = nextLevel.ToString();
            linesValue.text = linesDeleted.ToString();
            stageValue.text = (stage + 1).ToString();
